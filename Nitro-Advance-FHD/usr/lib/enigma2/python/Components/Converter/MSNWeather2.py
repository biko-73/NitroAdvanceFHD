...
weather_city = config.plugins.NitroAdvanceFHD.city.value
degreetype = config.plugins.NitroAdvanceFHD.degreetype.value
weather_location =  config.plugins.NitroAdvanceFHD.weather_location.value

if weather_location == 'en-EN':
	weather_location = 'en-US'

time_update = config.plugins.NitroAdvanceFHD.refreshInterval.value
#time_update = 30
time_update_ms = 3000
...
# Day 4
	...
	def write_none(self):
		with open("/tmp/iFlatweathermsn.xml", "w") as noneweather:
			noneweather.write("None")
		noneweather.close()

	def get_xmlfile(self):
		self.iConsole.ePopen("wget -P /tmp -T2 'http://weather.service.msn.com/data.aspx?weadegreetype=%s&culture=%s&weasearchstr=%s&src=outlook' -O /tmp/iFlatweathermsn.xml" % (degreetype, weather_location, weather_city), self.control_xml)
...
#
		if fileExists("/tmp/iFlatweathermsn.xml"):
			if int((time.time() - os.stat("/tmp/iFlatweathermsn.xml").st_mtime)/60) >= time_update:
				self.get_xmlfile()
		else:
			self.get_xmlfile()
		if not fileExists("/tmp/iFlatweathermsn.xml"):
			self.write_none()
			return info
		if fileExists("/tmp/iFlatweathermsn.xml") and open("/tmp/iFlatweathermsn.xml").read() == 'None':
			try:
				self.get_xmlfile()
			except:
				return info
		for line in open("/tmp/iFlatweathermsn.xml"):
			try:
				if "<weather" in line:
